# War Thunder UID Guard v0.9.0

v0.9.0 更新：圆角玻璃风格界面，按钮悬停流光与按压反馈；服务器延迟每秒刷新；修复语言下拉框关闭后仍保留蓝色选中高亮的问题，同时保留键盘选择与焦点提示。

Windows 桌面伴侣程序，用 UID 保存本地黑名单，并通过玩家昵称历史监听 War Thunder 的公开本地接口 `127.0.0.1:8111`。

本项目自主编写的代码采用 [MIT License](LICENSE) 开源。第三方组件继续遵循各自的许可证，详见 [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md)；隐私与网络访问范围见 [PRIVACY.md](PRIVACY.md)。
...
- 黑名单昵称出现在聊天或 HUD 战斗事件时，通过不占用系统托盘、不抢占焦点且自动关闭的右下角窗口提醒，并播放系统提示音。
...
勾选程序顶部的“远程同步”只会启用两个按钮，不会自动上传或拉取。
...
点击“检查更新”后，程序先读取专用服务器上经过 RSA-3072 签名的版本元数据；服务器存在有效新版本时立即开始服务器下载，不再等待 GitHub 查询。服务器不可用、没有新版本或文件下载失败时，才访问 `elainasamae/WarThunderUIDGuard` 的 GitHub 正式发布接口。GitHub 下载后备仍被服务器签名元数据中的 SHA-256 约束。服务器每 5 分钟同步最新正式版本、校验 GitHub 发布页提供的 SHA-256 后再生成签名元数据。更新校验无误后程序会退出，把正在运行的 EXE 所在目录原地更新为目标版本，并自动重启；新版本首次启动时会把专用程序目录自动改名为 `WarThunderUIDGuard-v对应版本-win-x64`。只有目录内存在与 EXE 版本一致的 `VERSION.txt` 时才允许改名，目标目录已经存在时不会覆盖。替换前后都会核对 EXE 版本，失败则保留或恢复当前版本。发布目录中的 `VERSION.txt` 和窗口标题会显示实际版本。黑名单位于 `%LOCALAPPDATA%`，目录改名和更新都不会删除它。v0.6.0 及更早版本首次升级到 v0.6.1 仍会使用 GitHub；从 v0.6.1 升级时已经优先使用专用服务器。
