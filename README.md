# War Thunder UID Guard v0.6.1 Safe
...
点击“检查更新”后，程序先读取专用服务器上经过 RSA-3072 签名的版本元数据，并优先下载服务器保存的免安装 ZIP；服务器不可用、文件缺失或 SHA-256 校验失败时，自动回退到 `elainasamae/WarThunderUIDGuard` 的 GitHub 正式发布接口。服务器每 5 分钟同步最新正式版本、校验 GitHub 发布页提供的 SHA-256 后再生成签名元数据。更新校验无误后程序会退出、替换文件并自动重启；失败则保留或恢复当前版本。黑名单位于 `%LOCALAPPDATA%`，更新不会删除它。v0.6.0 及更早版本首次升级到 v0.6.1 仍会使用 GitHub，此后版本即可优先使用专用服务器。
